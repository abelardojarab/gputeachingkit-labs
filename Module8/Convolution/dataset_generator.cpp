...
void compute(wbImage_t output, wbImage_t input, float *mask, int mask_rows, int mask_cols) {
      ...
      for (int c = 0; c < num_channels; ++c) { // channels
        ...
        for (int off_y = -mask_radius_y; off_y <= mask_radius_y; ++off_y) {
          for (int off_x = -mask_radius_x; off_x <= mask_radius_x; ++off_x) {
            ...
            if (in_y < img_height && in_y >= 0 &&
                in_x < img_width  && in_x >= 0) {
        ...
        //fprintf(stderr, "%f %f\n", clamp(acc));
        outputData[(out_y * img_width + out_x) * num_channels + c] = clamp(acc);
      }
    }
  }
}
...
void generate(int datasetNum, int height, int width, int minVal, int maxVal) {
  ...
  char *input_image_file_name = generateInput(datasetNum, dir_name, params);
  ...
  float *mask_data = (float *) wbImport(input_mask_file_name, &mask_rows, &mask_cols);
