...
static char base_dir[] = "./BfsQueue/Dataset";
...
static void write_data(char *file_name, unsigned int *data, unsigned int len) {
...
static void create_dataset(const int datasetNum, const int numNodes, const int maxNeighborsPerNode, Mode mode) {
  char dir_name[PATH_MAX];
  sprintf(dir_name, "%s/%d", base_dir, datasetNum);
  _mkdir(dir_name);

  // graph structure and current level (input data)
  char *mode_file_name = strjoin(dir_name, "/input0.raw");
  char *nodePtr_file_name = strjoin(dir_name, "/input1.raw");
  char *nodeNeighbors_file_name = strjoin(dir_name, "/input2.raw");
  char *nodeVisited_file_name = strjoin(dir_name, "/input3.raw");
  char *currLevelNodes_file_name = strjoin(dir_name, "/input4.raw");
  ...
  setupProblem(numNodes, maxNeighborsPerNode,
               &nodePtrs_h, &nodeNeighbors_h, &nodeVisited_h, &nodeVisited_ref,
               &currLevelNodes_h, &nextLevelNodes_h, &numCurrLevelNodes_h, &numNextLevelNodes_h);

  compute(numNodes, nodePtrs_h,
          nodeNeighbors_h,   nodeVisited_h,
          nodeVisited_ref, currLevelNodes_h,
          nextLevelNodes_h,  numCurrLevelNodes_h,
          numNextLevelNodes_h);


  write_data(nodePtr_file_name, nodePtrs_h, numNodes+1);
  write_data(nodeNeighbors_file_name, nodeNeighbors_h, nodePtrs_h[numNodes]);
  write_data(nodeVisited_file_name, nodeVisited_h, numNodes);
  write_data(currLevelNodes_file_name, currLevelNodes_h, *numCurrLevelNodes_h);
